...
if specorder > 1
	...
	IRcoordsstart(iv,:) = EDB2findis(R,iv,planeeqs,1,onesvec3);
    ...
    if ~isempty(nspecular_select)
		for ii = 1:specorder-1
            ...
            elseif ii == 2
                bigIRcoords(listselect,:) = EDB2findis(IRcoordsstart(PotentialISESshift(listselect,specorder),:),...
             ...
             elseif ii >= 3
                ...
                newIRcoordssimple    = EDB2findis(bigIRcoords(IRreflist(ivsimple),:),...
                                                  PotentialISESshift(listselect(ivsimple),specorder-(ii-1)),planeeqs,size(ivsimple,1),onesvec3);
                newIRcoordsnotsimple = EDB2findis(IRcoordsstart(PotentialISESshift(listselect(ivnotsimple),specorder),:),...
                                                  PotentialISESshift(listselect(ivnotsimple),specorder-1),planeeqs,size(ivnotsimple,1),onesvec3);
                for jj = 2:ii-1
                    newIRcoordsnotsimple = EDB2findis(newIRcoordsnotsimple,...
       ...
       for jj = ii:-1:1
            ...
            if nposs > 0
                ...
                if jj == ii
                    ...
                    [toedgecoords,edgeweightlist,edgenumberlist] = EDB2getedgepoints(edgestartcoords(possibleedges,:),edgeendcoords(possibleedges,:),edgelengthvec(possibleedges,:),nedgesubs);
                    ...
                    if isempty(ivnoIRexist)
                        ...
                        newIRcoords = EDB2findis(R,ISEScutout(:,ii),planeeqs,1,onesvec3);
                        for kk = 2:jj
                            newIRcoords = EDB2findis(newIRcoords,ISEScutout(:,ii-kk+1),planeeqs,size(newIRcoords,1),onesvec3);
                        end
                ...
                [hitplanes,reflpoints,edgehits,edgehitpoints,cornerhits,cornerhitpoints] = EDB2chkISvisible(fromcoords,tocoords,planeeqs(expandedposscombs(:,colno),4),planenvecs(expandedposscombs(:,colno),:),minvals(expandedposscombs(:,colno),:),...
