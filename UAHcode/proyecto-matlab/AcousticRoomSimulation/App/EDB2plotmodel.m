...
if plotnvecs
	for ii = 1:nplanes
        ...
        plot3(midpoint(1),midpoint(2),midpoint(3),'gx');
	end
end
...
if plotsources == 1
    if isempty(plotoptions2)
        ...
        else  %esta es la que se ejecutara en el mio
            num_sources=plotoptions2(:,1);
            zeros=find(num_sources == 0);
            num_sources(zeros)=[];
            for j=1:length(num_sources)   %mio
              plot3(sources(num_sources(j,1),1),sources(num_sources(j,1),2),sources(num_sources(j,1),3),'k*')
...
if plotreceivers == 1
    if isempty(plotoptions2)
        ...
        if nplotoptions2 == 1
            ...
            num_receivers=plotoptions2(:,2);
            zeros=find(num_receivers == 0);
            if zeros
            num_receivers(zeros)=[];
            end
             for j=1: length(num_receivers)  %mio
             plot3(receivers(num_receivers(j,1),1),receivers(num_receivers(j,1),2),receivers(num_receivers(j,1),3),'r*');
