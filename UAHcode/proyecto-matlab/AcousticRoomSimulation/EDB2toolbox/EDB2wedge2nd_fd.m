function tf = EDB2wedge2nd_fd(frequencies,cylS,cylR,cylE2_r1,cylE1_r2,...
nyveclist,edgelengthlist,dzvec,pathalongplane,R_irstart,bc,cair);
% EDB2wedge2nd - Gives the 2nd-order diffraction tf.
%
...
% yet.
%
%   tf     The 2nd order diffraction transfer function for two
...
% ----------------------------------------------------------------------------------------------
% Peter Svensson (svensson@iet.ntnu.no) 080508
%
% tf = EDB2wedge2nd_fd(frequencies,cylS,cylR,cylE2_r1,cylE1_r2,...
% nyveclist,edgelengthlist,dzvec,method,pathalongplane,BigB,R_irstart,bc,cair);
...
% edgeradialdistdiff1 = abs(rE1_r1-rE2_r1)/(mean([rE1_r1 rE2_r1])+10*eps);
% edgeradialdistdiff2 = abs(rE1_r2-rE2_r2)/(mean([rE1_r2 rE2_r2])+10*eps);

% edgesareparallel = edgeradialdistdiff1 < 1e-6 & edgeradialdistdiff2 < 1e-6;
...
%B4 = BIGEDGESTEPMATRIX(:,1)*edgelengthlist(1);      % B4 is zedge1_re1 which goes, per def., from 0 to edgelength1
if swapbigmatrix == 0,
    ...
    %B5 = BIGEDGESTEPMATRIX(:,2)*edgelengthlist(2);      % B5 is zedge2_re2 which goes, per def., from 0 to edgelength2
...
% min(min( B1- cos(nyveclist(1)*(pi + thetaS + thetaedge2_re1 ))  ))
% min(min( B1- cos(nyveclist(1)*(pi + thetaS - thetaedge2_re1 ))  ))
% min(min( B1- cos(nyveclist(1)*(pi - thetaS + thetaedge2_re1 ))  ))
% min(min( B1- cos(nyveclist(1)*(pi - thetaS - thetaedge2_re1 ))  ))
% 
% min(min( B2-cos(nyveclist(2)*(pi + thetaedge1_re2 + thetaR))  ))
% min(min( B2-cos(nyveclist(2)*(pi + thetaedge1_re2 - thetaR))  ))
% min(min( B2-cos(nyveclist(2)*(pi - thetaedge1_re2 + thetaR))  ))
% min(min( B2-cos(nyveclist(2)*(pi - thetaedge1_re2 - thetaR))  ))
...
%-------------------------------------------------------------------------
% Determine the  exp(-jk(m+l1+l2-Rirstart))

B2 = ( S2Edist + E2Edist + E2Rdist - R_irstart);

tf = zeros(nfrequencies,1);

for ii = 1:nfrequencies
    k = 2*pi*frequencies(ii)/CAIR;
    tf(ii) = sum(B3.*exp(-j*k*B2));
end
